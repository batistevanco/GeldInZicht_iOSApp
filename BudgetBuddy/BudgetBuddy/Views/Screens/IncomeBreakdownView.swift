...
// /Views/Screens/IncomeBreakdownView.swift
...
struct IncomeBreakdownView: View {
    ...
    let transactions: [Transaction]   // verwacht al gefilterd op periode indien gewenst
    ...
    private var slices: [DonutSlice] {
        let grouped = Dictionary(grouping: incomeTxs, by: { $0.category?.name ?? "Overig" })
